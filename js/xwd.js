...
(function xwdModule(global) {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      for (var clueNumber in this.clues[direction]) {
        if (this.clues[direction].hasOwnProperty(clueNumber)) {
          ...
          this.drawNumber(ctx, clueNumber, coord(clue.x, clue.y));
        }
      }
    }
  };
  ...
  Grid.prototype.drawLetters = function (ctx) {
    ...
    for (var key in this.letters) {
      ...
      this.drawLetter(ctx, letter, coordFromString(key));
    }
  };
  ...
  Grid.prototype.drawNumber = function (ctx, number, cell) {
    ...
    ctx.fillText(
      number,
      this.cellSize * (cell.x + 0.1),
      this.cellSize * (cell.y + 0.1),
    );
  };
  ...
  Grid.prototype.drawLetter = function (ctx, letter, cell) {
    ...
    ctx.fillText(
      letter,
      this.cellSize * (cell.x + 0.5) + 1,
      this.cellSize * (cell.y + 0.5) + 1,
    );
  };
  ...
  Grid.prototype.onClick = function (event, canvas, ctx) {
    var x = Math.floor((event.pageX - canvas.offsetLeft - 2) / this.cellSize);
    var y = Math.floor((event.pageY - canvas.offsetTop - 2) / this.cellSize);
    var cell = coord(x, y);
    if (cellInArray(this.whiteSquares, cell)) {
  ...
  Grid.prototype.highlightClueFromCell = function (cell, toggle) {
    ...
    if (cluesContainingCell.length === 0) {
      ...
      if (this.highlighted === null) {
        ...
        if (toggle) {
          if (
            this.highlighted.direction === cluesContainingCell[0].direction &&
            this.highlighted.number === cluesContainingCell[0].number
          ) {
  ...
  var Crossword = function (
    ...
    selectedClueDiv.addEventListener("clue-selected", function (event) {
      if (event.detail != null && "direction" in event.detail) {
        if (
          clueJson[event.detail.direction].hasOwnProperty(
            event.detail.clueNumber,
          )
        ) {
          var direction = event.detail.direction === "ac" ? "across" : "down";
          var clueString = self.clueToString(
            clueJson[event.detail.direction][event.detail.clueNumber],
          );
  ...
  Crossword.prototype.setupCanvas = function () {
    ...
    this.cellSize = Math.floor(
      Math.min(pixelWidth / AC_SQUARES, pixelHeight / DN_SQUARES),
    );
