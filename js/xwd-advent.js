...
(function adventXwdModule(global) {
      ...
      if (this.messageSquares.length > 0) {
        ...
        if (msgSqCoord.equals(cell)) {
          ...
          this.emitEvent("message", {
            detail: "Hidden message incorrect! Try again.",
          });
        }
      }
    }
  ...
  adventCrosswordProto.onCorrect = function () {
    ...
    if (this.messageSquares.length > 0) {
      this.grid.emitEvent("message", {
        detail: "Now highlight the hidden message.",
      });
    } else {
  ...
  function isClueActive(clue) {
    ...
    return (
      currentYear > intYear || (currentMonth === 11 && dayOfMonth >= clue[2])
    );
  }
